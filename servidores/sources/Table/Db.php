...
class _Db extends Table {
	...
	public function getRows($advancedSearchValues) {
		...
		if($this->sortBy and $this->isSqlSort($results) !== true) {
			$sortBy = $this->sortBy;
	...
	protected function isSqlSort($count) {
		if(!$this->sortBy) {
			return false;
		}
		...
		if(mb_strstr($this->sortBy, ',')) {
			...
			return true;
		} else if (array_key_exists(preg_replace("/^.+?\.(.+?)$/", "$1", $this->sortBy), $count)) {
			return true;
		}
