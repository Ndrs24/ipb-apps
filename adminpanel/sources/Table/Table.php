<?php
namespace IPS\adminpanel\Table;
...
abstract class _Table
{
	 ...
	 * @note	Keys are the field names. Values are a IPS\adminpanel\Table\SEARCH_* constant
	 */
	 ...
	 * @see		\IPS\adminpanel\Table\getRows
	 */
	public $pages = 1;
	 ...
	 * @code
	 	...
	 	{
	 		return array( ... ); // Same format as IPS\adminpanel\Table::$rootButtons
	 	}
	 * @endcode
	 */
	...
	protected function _convertSearchValuesForQueryString( $values )
	{
		...
		foreach( $values as $k => $v )
		{
			...
			else if( isset( $this->advancedSearch[ mb_substr( $k, mb_strlen( $this->langPrefix ) ) ] ) and $this->advancedSearch[ mb_substr( $k, mb_strlen( $this->langPrefix ) ) ] === \IPS\adminpanel\Table\SEARCH_BOOL )
			{
