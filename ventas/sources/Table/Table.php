...
abstract class _Table
{
	 ...
	 * @note	Keys are the field names. Values are a IPS\ventas\Table\SEARCH_* constant
	 */
	 ...
	 * @code
	 	...
	 	{
	 		return array( ... ); // Same format as IPS\ventas\Table::$rootButtons
	 	}
	 * @endcode
	 */
	...
	protected function _convertSearchValuesForQueryString( $values )
	{
		...
		foreach( $values as $k => $v )
		{
			...
			else if( isset( $this->advancedSearch[ mb_substr( $k, mb_strlen( $this->langPrefix ) ) ] ) and $this->advancedSearch[ mb_substr( $k, mb_strlen( $this->langPrefix ) ) ] === \IPS\ventas\Table\SEARCH_BOOL )
			{
