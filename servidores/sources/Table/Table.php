<?php
namespace IPS\servidores\Table;
...
abstract class _Table
{
	 ...
	 * @note	Keys are the field names. Values are a IPS\servidores\Table\SEARCH_* constant
	 */
	 ...
	 * @see		\IPS\servidores\Table\getRows
	 */
	public $pages = 1;
	 ...
	 * @code
	 	...
	 	{
	 		return array( ... ); // Same format as IPS\servidores\Table::$rootButtons
	 	}
	 * @endcode
	 */
	...
	protected function _convertSearchValuesForQueryString( $values )
	{
		...
		foreach( $values as $k => $v )
		{
			...
			else if( isset( $this->advancedSearch[ mb_substr( $k, mb_strlen( $this->langPrefix ) ) ] ) and $this->advancedSearch[ mb_substr( $k, mb_strlen( $this->langPrefix ) ) ] === \IPS\servidores\Table\SEARCH_BOOL )
			{
